...
func testAccCheckPublicCloudUserExists(n string, t *testing.T) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		return pcuExists(rs.Primary.Attributes["project_id"], rs.Primary.ID, config.OVHClient)
	}
}
...
func testAccCheckPublicCloudUserDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		err := pcuExists(rs.Primary.Attributes["project_id"], rs.Primary.ID, config.OVHClient)
		if err == nil {
