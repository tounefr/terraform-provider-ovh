...
func testAccCheckPublicCloudPrivateNetworkSubnetExists(n string, t *testing.T) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		return pcpnsExists(rs.Primary.Attributes["project_id"], rs.Primary.Attributes["network_id"], rs.Primary.ID, config.OVHClient)
	}
}
...
func testAccCheckPublicCloudPrivateNetworkSubnetDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		err := pcpnsExists(rs.Primary.Attributes["project_id"], rs.Primary.Attributes["network_id"], rs.Primary.ID, config.OVHClient)
		if err == nil {
